...
def getInfo(hotesListe,nouv_fich,aff_err,port,ch_fich):
    for i in hotesListe:                               #On parcourt la liste des @ des hôtes entrés par l'utilisateur
        ...
        try :
            client_socket.connect((str(i), port))                                                        #Essaye de se connecter à l'hôte sur le port spécifié
            message_recu = client_socket.recv(1024).decode() 
            if ch_fich.upper()=="OUI":
                nouv_fich.write("["+str(i)+"] :"+message_recu+"\n")                                       #Decode le message reçu de l'hôte
...
def ip_liste():
    ...
    #-----------  SCAN -------------
    getInfo(hotesListe,nouv_fich,aff_err,port,ch_fich)
...
def ip_local():
    ...
    #-----------  SCAN -------------
    getInfo(listing_ip,nouv_fich,aff_err,port,ch_fich)
...
def ip_fichier():                       #Par défaut, variable aff_err en OUI pour afficher toutes les erreurs
    ...
    #----------- SCAN -------------
    getInfo(listing_ip_fich,nouv_fich,aff_err,port,ch_fich)
...
\ \| | | / __|   \ \ / __/ _` | '_ \ 
_\ \ |_| \__ \   _\ \ (_| (_| | | | |
\__/\__, |___/___\__/\___\__,_|_| |_|v1.2
    |___/ 
    """)
    print("--------------------------------------------"+Colors.END)
